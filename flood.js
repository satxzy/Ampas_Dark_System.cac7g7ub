// flood.js - FLOOD GITHUB BY RYUICHI (Rebuild)
// Upload folder ke banyak repo GitHub + clean tool
...
${c.yellow}${c.b}📋  OPSI TAMBAHAN:${c.r}
  ${c.green}--ignore${c.r} <file>   Abaikan file/folder (bisa diulang)
                   Otomatis skip: flood.js, .git, node_modules

${c.yellow}${c.b}📝  CONTOH:${c.r}
  # Upload 10 repo dengan nama "proyek"
  node flood.js --token ghp_xxx --name proyek --count 10

  # Upload 5 repo private, safe mode, folder ./data
  node flood.js --token ghp_xxx --name backup --count 5 --folder ./data --private --safe
...
${c.yellow}${c.b}💡  TIPS:${c.r}
  ...
  - Jika error socket hang, script akan retry otomatis.
`);
  process.exit(0);
}
...
const config = {
  ...
  baseName: 'repo',
  count: 1,
  concurrency: 3,       // lebih aman
  delay: 1500,          // jeda antar repo
  folder: '.',
  private: false,
  safeMode: false,
  fileDelay: 200,
...
(async () => {
  ...
  console.log(`${c.gray}📁  Folder: ${path.resolve(config.folder)}${c.reset} ${c.green}(${allFiles.length} file, ${formatSize(totalSize)})${c.reset}`);
